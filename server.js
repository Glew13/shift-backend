...
// Scrape Motors.co.uk search results
async function scrapeMotors(url) {
    try {
        console.log('Scraping Motors.co.uk:', url);
...
app.post('/api/search', async (req, res) => {
    try {
        ...
        const cars = await scrapeMotors(url);
        ...
        res.json({
            ...
            count: cars.length,
            cars,
            source: cars.length > 0 && cars[0].url.includes('motors.co.uk') ? 'motors.co.uk' : 'demo_data'
        });
...
app.listen(PORT, () => {
    ...
    console.log(`🌐 Attempting to scrape from Motors.co.uk`);
});
